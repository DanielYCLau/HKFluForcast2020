...
for (k in 1:4) {
  ...
  for (i in c("WIS", "RMSE", "LNQ", "MAE")) { 
    ...
    a.r = sapply(
      expected.y[as.character(2011:2018)], 
      function(x) { 
    ...
    for (p in period) {
      ...
      if (p %in% c("winter", "all")) {
        ...
        f.att.PI = (d.att + c(f.att, f.mc)) / 1e6 * 100
        f.att.CI = (d.att + c(f.att, f.CI)) / 1e6 * 100
      } else {
        f.att.PI = c(f.att, f.mc) / 1e6 * 100
        f.att.CI = c(f.att, f.CI) / 1e6 * 100
      }
      
      # reduction in attack rate
      ILIpS.p = subset(ILIpS, eval(parse(text = subset.p)))
      d.att = sum(ILIpS.p[, "ILIpS"]) / 1e6 * 100
