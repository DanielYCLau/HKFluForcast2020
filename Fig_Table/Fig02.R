...
event.fx = function(event, event.t, event.y, ...) {
  ...
  segments(x0 = event.t, y0 = -1, y1 = event.y)
  shape::Arrows(
    x0 = event.t, 
    x1 = event.t + 0.02, 
    y0 = event.y, 
...
pdf(
  height = 8, 
  width  = 6, 
  file   = file.fx("Fig2_short_long_forec.pdf")
)
...
for (f.period in c(4, 13)) {
  ...
  with(forec.y.ki, polygon.fx(t, fit.mean.LB, fit.mean.UB, col = col.fx(i, 0.25)))
