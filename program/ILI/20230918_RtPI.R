...
pred.Rt = foreach(
  ...
  .options.snow = pb.fx(nrow(pred.y.mc))
) %dopar% {
...
save(
  pred.Rt,
  file = file.fx(sprintf("reg_sep_fore_%02dw_%d_forecast_%dw_Rt.Rdata", f.period, f.yr, f.start))
)
...
write.csv(
  Rt.compare, 
  file = file.fx(sprintf("reg_sep_fore_13w_2020_forecast_%dw_Rt.csv", f.start)), 
  row.names = F
)
