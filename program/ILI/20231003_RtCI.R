...
pred.Rt = foreach(
  ...
  .options.snow = pb.fx(nrow(pred.mu.mc))
) %dopar% {
  
  inc.data.i = inc.data
  ind = (4 + f.start):nrow(inc.data.i)
  # 20230918 use pred.y.mc for PI
...
save(
  pred.Rt,
  file = file.fx(sprintf("reg_sep_fore_%02dw_%d_forecast_%dw_RtCI.Rdata", f.period, f.yr, f.start))
)
