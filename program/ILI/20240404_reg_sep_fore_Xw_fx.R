...
nb.llh.fx = function(pars, y, X) {
  ...
  sum.llh = ifelse(is.infinite(sum.llh), -1e3 * length(y), sum.llh)
  ...
  pred.y.list = lapply(
    ...
    function(t) {
      ...
      fit.t.2 = optim(
        ...
        y       = fit.t[["y"]],
        X       = fit.t[["x"]],
        control = list("ndeps" = rep(1e-5, length(coef(fit.t))+1 ) )
      )
...
criteria.rank.fx = function(cv, rank.w.value) {
  ...
  rank.c = lapply(1:nrow(cv), function(i) { sprintf("%d (%.01f)", rank[i,], cv[i, criteria.col.1]) } )
