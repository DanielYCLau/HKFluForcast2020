...
N = nrow( f.list[[1]][["result"]][["WIS"]][["forec"]][["pred.y.mc"]] )
...
pred.Rt = foreach(
  ...
  pred.y.mc = f.list[[f.start]][["result"]][["WIS"]][["forec"]][["pred.y.mc"]]
  
  inc.data.i = inc.data
  ind = (4 + f.start):nrow(inc.data.i)
  inc.data.i[ind, "fit.mean"] = pred.y.mc[i, 1:length(ind)]
...
save(
  pred.Rt,
  file = file.fx(sprintf("reg_sep_fore_%02dw_%d_forecast_RtPI.Rdata", f.period, f.yr))
)
...
write.csv(
  Rt.compare, 
  file = file.fx(sprintf("reg_sep_fore_13w_2020_forecast_%dw_RtPI.csv", f.start)), 
  row.names = F
)
