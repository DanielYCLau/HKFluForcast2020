...
data[, "ILIpS"] = with(data, ifelse(ILIpS == 0, 0.1, ILIpS))
...
train.st = seq(as.Date("2014-01-14"), by = "week", length.out = length(train.ed))
...
load(list.files(pattern = sprintf("reg_sep_fore_%02dw_%d_CV_result.Rdata", f.period, f.yr), full.names = T))
...
save(
  arima.cv, 
  file = sprintf("arima/%s", file.fx(sprintf("arima_fore_%02dw_CV.Rdata", f.period))) 
)
...
file     = list.files("arima/linux", sprintf("arima_fore_%02dw_CV.*Rdata", f.period), full.names = T)
arima.cv = lapply(file, function(x) { load(x); return(arima.cv) })
...
arima.cv = foreach(
  ...
  if (bs.k == 0) {
    t.reg.i = t.reg.i[, !grepl("M_bs", colnames(t.reg.i)), drop = F]
    v.reg.i = v.reg.i[, !grepl("M_bs", colnames(v.reg.i)), drop = F]
  }
  n.x = ncol(t.reg.i)
...
file = list.files("arima", sprintf("arima_fore_%02dw_CV.*Rdata", f.period), full.names = T)
load(file)
...
arima.cv = with(
  ...
  data.frame(
    "model"  = sprintf(
      "ARIMA (%d,%d,%d)(%d,%d,%d)%s+%s", 
      AR,D,MA,sAR,sD,sMA, 
      ifelse(bs == 1, "+bs(12)", ""),
      paste(reg.x[!grepl("M_bs", reg.x)], collapse = "+")
    ), 
    "v.RMSE" = RMSE, 
    "v.LNQ"  = LNQ, 
    "v.MAE"  = MAE, 
...
write.csv(
  cv.table, 
  file = sprintf("arima/%s", file.fx(sprintf("arima_fore_%02dw_CV.csv", f.period))) 
)
