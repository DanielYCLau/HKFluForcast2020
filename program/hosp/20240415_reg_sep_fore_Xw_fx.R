...
pomp.v = as.numeric( substr(packageVersion("pomp"), 1, 1) )
...
data.fx = function(data, f.yr, f.period, lag, start = 2010) {
  ...
  if (pomp.v < 5) {
    t.lag.1.M.bs  = periodic.bspline.basis(dec.t.lag.1, nbasis = 12, names = "t_lag_1_M_bs_%02d")
  } else {
    t.lag.1.M.bs  = periodic_bspline_basis(dec.t.lag.1, nbasis = 12, names = "t_lag_1_M_bs_%02d")
  }
  # t.lag.1.M.bs  = periodic.bspline.basis(dec.t.lag.1, nbasis = 26, names = "t_lag_1_M_bs_%02d")
...
pred.fx = function(model, data, pred.X, alpha, keep.mc) {
  ...
  pred.y.list = lapply(
    ...
    function(t) {
      ...
      fit.t     = try( suppressWarnings(glm.nb(model.t, data = data, x = T, maxit = 1000)), silent = T )
      
      
      if (length(class(fit.t)) < 3) {
      ...
      } else if (fit.t[["theta"]] > 1e6 | fit.t[["SE.theta"]] > 1e3) {
        ...
        while (fit.ind) {
          ...
          fit.t.2 = suppressWarnings( try(
            optim(
              par     = c(coef(fit.t), log( fit.t[["theta"]] ) ),
              # par     = c(coef(fit.t), log( fit.t[["theta"]] ) ),
              fn      = nb.llh.fx,
              method  = "L-BFGS-B",
              hessian = T,
              y       = fit.t[["y"]],
              X       = fit.t[["x"]],
              upper   = c(rep(15, length(coef(fit.t))), log(1e6)),
              control = list("ndeps" = rep(ndeps, length(coef(fit.t))+1 ), "maxit" = 1e6 )
            ), silent = T
          ) )
          
          fit.ind = ( length(fit.t.2) == 1 ) & ndeps < 1
          if ( !fit.ind ) {
            fit.t.cov = try(solve(fit.t.2[["hessian"]]), silent = T)
            if (is.matrix(fit.t.cov)) {
              fit.ind   = any(diag(fit.t.cov) < 0) | any(eigen(fit.t.cov)[["values"]] < 0)
            } else {
              fit.ind   = T
            }
          }
      ...
      # fit.t.est = fit.t[["coefficients"]]
      # fit.t.cov = solve(fit.t[["hessian"]])
